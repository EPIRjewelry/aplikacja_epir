...
**Cursor IDE** uses the same docs plus **`EPIR_AI_BIBLE.md`** (router SSOT), **`docs/kb/`** (moduły wiedzy agentowej), **`.cursor/index.mdc`** and **`.cursor/rules/`** — see `AGENTS.md` § Cursor. Copilot agent personas live in **`.github/agents/`**; do not fork architecture text into a third place.
