...
# 1. npm ci (workspaces)
Write-Host "`n[1/9] npm ci..." -ForegroundColor Yellow
Set-Location $root
npm ci
if ($LASTEXITCODE -ne 0) { throw "npm ci failed" }

# 2. Deploy workers (rag → analytics → bigquery-batch → analyst → marketing-ingest → chat; chat wymaga rag + analytics).
#    wrangler deploy --env="" = jawny top-level przy [env.*] w wrangler.toml (zalecenie CLI).
Write-Host "`n[2/9] Deploy epir-rag-worker..." -ForegroundColor Yellow
Set-Location "$root\workers\rag-worker"
wrangler deploy --env=""
if ($LASTEXITCODE -ne 0) { throw "rag deploy failed" }

Write-Host "`n[3/9] Deploy epir-analityc-worker..." -ForegroundColor Yellow
Set-Location "$root\workers\analytics"
wrangler deploy --env=""
if ($LASTEXITCODE -ne 0) { throw "analytics deploy failed" }

Write-Host "`n[4/9] Deploy epir-bigquery-batch..." -ForegroundColor Yellow
Set-Location "$root\workers\bigquery-batch"
wrangler deploy --env=""
if ($LASTEXITCODE -ne 0) { throw "bigquery-batch deploy failed" }

Write-Host "`n[5/9] Deploy epir-analyst-worker..." -ForegroundColor Yellow
Set-Location "$root\workers\analyst-worker"
wrangler deploy --env=""
if ($LASTEXITCODE -ne 0) { throw "analyst-worker deploy failed" }

Write-Host "`n[6/9] Deploy epir-marketing-ingest..." -ForegroundColor Yellow
Set-Location "$root\workers\marketing-ingest"
wrangler deploy --env=""
if ($LASTEXITCODE -ne 0) { throw "marketing-ingest deploy failed" }

Write-Host "`n[7/9] Deploy epir-art-jewellery-worker (chat)..." -ForegroundColor Yellow
...
Write-Host "`n[8/9] shopify app build (repo @shopify/cli via npm script)..." -ForegroundColor Yellow
Set-Location $root
npm run shopify:app:build
if ($LASTEXITCODE -ne 0) { throw "shopify app build failed" }

Write-Host "`n[9/9] shopify app deploy..." -ForegroundColor Yellow
