...
**Cron:** `0 */6 * * *` (UTC) — patrz `workers/marketing-ingest/wrangler.toml`. Publicznie: `GET /`, `GET /healthz`. **`GET /feed/gmc_feed.csv`** — prebuilt CSV z R2 `epir-gmc-feed` (pipeline Node `epir-marketing-ingest/`, bez Shopify w GET). Z `workers_dev = true` worker ma też URL **`*.workers.dev`** (użyteczne do `GET /ops/marketing-preview`). Opcjonalnie **`GET /ops/marketing-preview`** — podgląd JSON (GA4 + Google Ads, bez ingestu), tylko gdy ustawisz sekret `MARKETING_OPS_PREVIEW_KEY` i nagłówek `Authorization: Bearer …`; bez sekretu ścieżka zwraca `404`.
