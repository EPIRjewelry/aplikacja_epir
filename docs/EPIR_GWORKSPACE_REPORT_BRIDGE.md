...
2. Raport trafia do D1 `operator_daily_reports` (baza `ai-assistant-sessions-db`) — **pełna treść, bez maskowania**.
3. Opcjonalnie: `POST` na `GWORKSPACE_REPORT_WEBHOOK_URL` — payload z **maskowaniem PII (SHA-256)** przed wysyłką (`report-pii-mask.ts`).
