...
Shopify dokleja do żądania parametry weryfikacji App Proxy (np. `shop`, `timestamp`, `signature` — **dokładne nazwy i zestaw** są opisane w oficjalnej dokumentacji Shopify *Authenticate app proxies*; **nie utrwalaj w RAG sztywnej listy** parametrów, jeśli nie cytujesz aktualnej dokumentacji).
...
- przy pytaniach w stylu *„Show me my past orders”* / analityka / architektura — asystent musi wyjaśnić ograniczenia i wrócić do zakresu (biżuteria, polityki).
...
5. **Parametry query Shopify:** nie inventaryzuj na sztywno — odsyłaj do dokumentacji Shopify *Authenticate app proxies*, worker kanonizuje **to, co przyjdzie** w URL (poza wykluczonymi kluczami).
...
*Dokument pomocniczy dla RAG. Aktualizuj przy zmianach w `shopify.app.toml`, `workers/chat` (zwłaszcza `security.ts`, `index.ts`, SessionDO, `luxury-system-prompt.ts`), loaderach Hydrogen lub `packages/ui/ChatWidget.tsx`.*
