/**
 * EPIR - InPost Geowidget v4: save pickup point to cart attributes (Leaflet, no custom elements).
...
(function () {
  ...
  // Search functionality (server-side via worker)
  ...
  function bindSearch(root) {
    ...
    root._epirSearchAbort = null;

    searchInput.addEventListener('input', function () {
      clearTimeout(searchDebounce);
      searchDebounce = setTimeout(function () {
        searchPoints(root, searchInput.value.trim());
      }, 300);
    });
  ...
  function hideSearchResults(searchResults) {
    searchResults.classList.remove('epir-inpost__search-results--visible');
    searchResults.hidden = true;
  }

  function showSearchResults(searchResults) {
    searchResults.classList.add('epir-inpost__search-results--visible');
    searchResults.hidden = false;
  }

  function showSearchLoading(searchResults) {
    searchResults.innerHTML = '<div class="epir-inpost__search-result-item">Szukam...</div>';
    showSearchResults(searchResults);
  }

  function renderSearchResults(root, matches, searchResults) {
    if (matches.length === 0) {
      searchResults.innerHTML = '<div class="epir-inpost__search-result-item">Brak wynikow — sprawdz kod lub wpisz wiecej liter</div>';
      showSearchResults(searchResults);
      renderMarkers(root, []);
      return;
    }
    ...
    searchResults.querySelectorAll('[data-epir-inpost-select]').forEach(function (el) {
      el.addEventListener('click', function () {
        ...
        var point = matches.find(function (p) { return p.code === code; });
        if (point) {
  ...
  function searchPoints(root, query) {
    ...
    // Code-like signatures (KRA / KRA01M) search from 2 chars; city/street same threshold
    if (query.length < 2) {
      searchResults.innerHTML = '<div class="epir-inpost__search-result-item">Wpisz min. 2 znaki (kod, miasto, ulica)</div>';
      showSearchResults(searchResults);
      return;
    }

    if (root._epirSearchAbort) {
      root._epirSearchAbort.abort();
    }
    root._epirSearchAbort = new AbortController();

    showSearchLoading(searchResults);

    fetch(WORKER_URL + '/points?country=PL&query=' + encodeURIComponent(query), {
      signal: root._epirSearchAbort.signal,
      credentials: 'same-origin'
    })
      .then(function (res) {
        if (!res.ok) throw new Error('Server error: ' + res.status);
      ...
      .then(function (data) {
        ...
        renderSearchResults(root, matches, searchResults);
      })
      .catch(function (err) {
        if (err && err.name === 'AbortError') return;
        searchResults.innerHTML = '<div class="epir-inpost__search-result-item">Nie udalo sie wyszukac punktow</div>';
        showSearchResults(searchResults);
      });
  }
