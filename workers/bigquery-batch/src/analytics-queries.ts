 ...
 *
 * Zdarzenia sklepu (pixel): tabela `epir_pixel_events_raw` (eksport z D1 przez epir-bigquery-batch),
 * nie legacy `events_raw` w tym samym datasetcie.
 */
...
purchase_sessions AS (
  SELECT DISTINCT session_id FROM \`analytics_435783047.epir_pixel_events_raw\`
...
UNION ALL
SELECT 'without_chat',
  (SELECT COUNT(DISTINCT session_id) FROM \`analytics_435783047.epir_pixel_events_raw\`) - (SELECT COUNT(*) FROM chat_sessions),
...
FROM \`analytics_435783047.epir_pixel_events_raw\`
...
WITH product_sessions AS (SELECT DISTINCT session_id FROM \`analytics_435783047.epir_pixel_events_raw\` WHERE event_type = 'product_viewed'),
purchase_sessions AS (SELECT DISTINCT session_id FROM \`analytics_435783047.epir_pixel_events_raw\` WHERE event_type = 'purchase_completed')
