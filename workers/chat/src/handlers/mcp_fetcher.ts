...
export interface MCPFetcherOptions {
  ...
  /** MCP endpoint path (default: '/api/mcp') - ignorowane gdy mcpEndpoint podany */
  endpointPath?: string;
  
  /** Pełny URL MCP - zmienna z wrangler.toml [vars] MCP_ENDPOINT (preferowane) */
  mcpEndpoint?: string;
...
export async function fetchMCP(
  ...
  const {
    ...
    mcpEndpoint: mcpEndpointOpt,
    timeout = 10000,
    method = 'search_dev_docs'
  } = opts || {};
  
  // Construct MCP endpoint URL - preferuj zmienną mcpEndpoint (z wrangler vars)
  const mcpUrl = mcpEndpointOpt?.trim() || `https://${shopDomain}${endpointPath}`;
