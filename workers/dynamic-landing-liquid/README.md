...
Search per grupa (3 aktywne): `utm_campaign=artisan_rings` | `forest_premium` | `artisan_gold`  
Rezerwa: `organic_art`. `artisan_new` — **usunięty z konfiguracji routingu** (kod + seed `APEX_MAPPING_REMOVED_KEYS`).

### Wizualna architektura (2026-08-12)

Moduły w `src/`: `shopify-cdn.ts`, `hero-picture.ts`, `stone-profile-hero.ts`; CSS w `render-shared.ts` (`.texture-organic`, `.kazka-bridge`); mosty w `landing-sections.ts`.  
SSOT operacyjny + URL-e live + kolejny krok (grafiki): [`docs/working/GENIALNY_PLAN_CURSOR_GROKA_4_5.md`](../../docs/working/GENIALNY_PLAN_CURSOR_GROKA_4_5.md).

Deploy Kazka Pages: `npm run deploy -w kazka` (root `pages:deploy:kazka` deleguje tutaj).
...
| `forest_premium` | `forest-premium-landing` | **Editorial** — manifesto + foundry + bestsellery + most → rings (`render-apex-editorial.ts`) |
| `artisan_rings` | `artisan-rings-landing` | j.w. (bridal / pierścionki + grawer + most → forest) |
| `artisan_gold` | `artisan-gold-landing` | j.w. (Technical Authority + Digital Co-creation + szept → Kazka) |
