...
/** Redirect to first curated product — never apex home when Ads traffic expects PDP path. */
async function redirectToFirstCampaignProduct(
  request: Request,
  env: Env,
  url: URL,
  ...
  return new Response('Campaign landing unavailable — no product redirect target', {
    status: 410,
    headers: {
    ...
    if (adsHost) {
      ...
      try {
        ...
        const handle =
          ...
          mapping.default;
        if (!handle) {
          return redirectToFirstCampaignProduct(request, env, url, mapping.forest_premium, {
            'X-EPIR-Landings-Enabled': 'true',
          });
        }
        ...
        return redirectToFirstCampaignProduct(request, env, url, handle, {
          'X-EPIR-Landings-Enabled': 'true',
        });
      } catch {
        return redirectToFirstCampaignProduct(request, env, url, null, {
          'X-EPIR-Landings-Enabled': 'true',
        });
      }
    }
