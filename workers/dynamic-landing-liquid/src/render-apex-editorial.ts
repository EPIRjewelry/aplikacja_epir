...
function renderCocreateTeaser(copy: {
  ...
  return `<section id="wspoltworzenie" class="panel-cream py-20 md:py-28 border-t border-epir-accent/10" aria-labelledby="cocreate-heading">
    <div class="mx-auto max-w-3xl px-5 md:px-8 text-center">
      ...
      <a href="https://epirbizuteria.pl/pages/kontakt" class="btn-cta inline-flex items-center justify-center px-8 py-3.5 rounded-full font-semibold text-sm tracking-wide text-epir-on">
...
/** Full editorial for forest_premium, artisan_rings, artisan_new, artisan_gold. */
export function renderApexEditorialHtml(
  ...
  const collectionUrl = absoluteStoreUrl(env, campaign.ctaUrl || '/');
  const featured = products[0];
  const heroCls = heroSectionClass(copy.heroMode);
  const visualCls = heroVisualClass(copy.heroMode);
  const featuredImg = featured?.featuredImage?.url
    ? `<img src="${escapeHtml(featured.featuredImage.url)}" alt="${escapeHtml(featured.featuredImage.altText || featured.title || copy.featuredTitle)}" class="w-full h-full object-cover" loading="eager" />`
    : `<div class="text-center p-8">
        <p class="font-serif text-epir-muted text-sm">${escapeHtml(copy.featuredTitle)}</p>
      </div>`;
...
<body class="font-sans text-epir-ink antialiased">
  ${renderNav(store)}
  <main>
    <section class="pt-24 md:pt-32 pb-16 md:pb-24 ${heroCls}" aria-labelledby="hero-heading">
    ...
    <section class="py-20 md:py-28" aria-labelledby="featured-heading">
      <div class="mx-auto max-w-6xl px-5 md:px-8">
        <div class="grid md:grid-cols-2 gap-12 md:gap-16 items-center">
          ...
          <div>
            ...
            ${
              featured?.handle
                ? `<a href="${escapeHtml(absoluteStoreUrl(env, `/products/${featured.handle}`))}" class="btn-cta inline-flex mt-8 px-8 py-3.5 rounded-full font-semibold text-sm tracking-wide text-epir-on">Zobacz produkt</a>`
