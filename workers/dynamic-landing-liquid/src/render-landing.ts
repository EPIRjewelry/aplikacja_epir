...
export function renderStandaloneLandingHtml(
  ...
  if (campaign.handle === ORGANIC_ART_HANDLE) {
    return renderOrganicArtLandingHtml(env, campaign, products);
  }
  if (APEX_EDITORIAL_HANDLES.has(campaign.handle)) {
    return renderApexEditorialHtml(env, campaign, products);
  }
  return renderCampaignLandingHtml(env, campaign, products);
}
