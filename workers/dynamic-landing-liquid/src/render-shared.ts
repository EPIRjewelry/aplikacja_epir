...
import {appendAttributionParams} from './landing-attribution';
...
/** Shopify Files favicon (Settings → Favicon). Update path if Admin replaces the file. */
export const STORE_FAVICON_CDN_PATH =
  '/cdn/shop/files/projekt_favicon_020b70c2-80c5-42ac-a687-fc71e2ddf465.png';

/** Same favicon links as epirbizuteria.pl Online Store head. */
export function renderStoreFaviconLinks(origin: string): string {
  const base = `${origin.replace(/\/$/, '')}${STORE_FAVICON_CDN_PATH}`;
  const icon32 = `${base}?crop=center&height=32&v=1728288571&width=32`;
  const apple180 = `${base}?crop=center&height=180&v=1728288571&width=180`;
  return `<link rel="icon" type="image/png" href="${escapeHtml(icon32)}" />
  <link rel="apple-touch-icon" href="${escapeHtml(apple180)}" />`;
}

function originFromCanonical(canonical: string): string {
  try {
    return new URL(canonical).origin;
  } catch {
    return 'https://epirbizuteria.pl';
  }
}
...
export function renderEditorialHead(opts: {
  ...
  ${renderStoreFaviconLinks(originFromCanonical(opts.canonical))}
  ...
  <style>
    ...
    .texture-organic::after {
      ...
      background-image: url("data:image/svg+xml,%3Csvg viewBox='0 0 512 512' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='t'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.35' numOctaves='3' stitchTiles='stitch'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23t)'/%3E%3C/svg%3E");
    }
    .texture-organic > * {
      position: relative;
      z-index: 2;
    }
    .kazka-bridge--whisper {
      border-top: 1px solid color-mix(in srgb, var(--epir-accent) 10%, transparent);
    ...
    .foundry-table th,
    .tech-authority-table th {
      font-size: 0.7rem;
      letter-spacing: 0.08em;
      text-transform: uppercase;
    }
    .silver-cross-bridge {
      border-top: 1px solid color-mix(in srgb, var(--epir-accent) 12%, transparent);
    }
    .reveal {
      opacity: 0;
    ...
    .viewer-placeholder {
      background: linear-gradient(145deg, var(--epir-bg-cream) 0%, var(--epir-bg-secondary) 55%, color-mix(in srgb, var(--epir-accent) 12%, white) 100%);
      box-shadow: 0 24px 60px rgba(0, 0, 0, 0.08);
    }
    .viewer-grid {
      background-image:
        linear-gradient(color-mix(in srgb, var(--epir-accent) 8%, transparent) 1px, transparent 1px),
        linear-gradient(90deg, color-mix(in srgb, var(--epir-accent) 8%, transparent) 1px, transparent 1px);
      background-size: 28px 28px;
    }
    .workshop-tile {
      background: linear-gradient(160deg, var(--epir-bg-secondary) 0%, var(--epir-bg-cream) 100%);
    }
    .workshop-shimmer {
    ...
    @media (prefers-reduced-motion: reduce) {
      ...
      .workshop-shimmer { animation: none; }
    }
  </style>`;
}
  ...
  return `<header class="glass-nav fixed top-0 inset-x-0 z-50">
    <nav class="mx-auto max-w-6xl px-5 md:px-8 flex items-center justify-between h-16" aria-label="Główna nawigacja">
      ...
      <ul class="hidden md:flex items-center gap-8 text-xs tracking-[0.15em] uppercase text-epir-muted">
        ...
        <li><a href="https://epirbizuteria.pl/pages/zaprojektuj-swoj-model" class="btn-cta font-semibold px-5 py-2 rounded-full text-[0.7rem] tracking-wider text-epir-on">Zaprojektuj Swój Model</a></li>
      </ul>
    ...
    <div id="mobile-menu" hidden class="md:hidden glass-nav border-t border-epir-accent/10 bg-white">
      <ul class="flex flex-col px-5 py-4 gap-4 text-sm tracking-widest uppercase text-epir-ink">
        ...
        <li><a href="https://epirbizuteria.pl/pages/zaprojektuj-swoj-model" class="mobile-link block py-2 text-epir-accent font-semibold">Zaprojektuj Swój Model</a></li>
      </ul>
    </div>
  </header>`;
}
...
export function renderProductStrip(opts: {
  ...
  const cards = products
    .map((p) => {
      const rawHref = absoluteStoreUrl(opts.env, `/products/${p.handle || ''}`);
      const href = attrSearch
        ? appendAttributionParams(rawHref, attrSearch)
        : rawHref;
  ...
  const moreHref = attrSearch
    ? appendAttributionParams(opts.moreHref, attrSearch)
    : opts.moreHref;

  return `<section id="kolekcje" class="py-20 md:py-28 border-t border-epir-accent/10 reveal" aria-labelledby="kolekcje-heading">
    <div class="mx-auto max-w-6xl px-5 md:px-8">
      ...
      <div class="mt-10 text-center">
        <a href="${escapeHtml(moreHref)}" class="btn-outline inline-flex items-center justify-center px-8 py-3.5 rounded-full font-medium text-sm tracking-wide" data-dynamic-cta-more>
